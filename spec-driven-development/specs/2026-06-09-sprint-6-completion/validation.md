...
- [ ] **R1 -- SDD-019.R7 closed: priority-weighted FIFO queue ordering.**
...
- [ ] **R2 -- SDD-019.R8 closed: cutover-commit grandfather migration.**
...
- [ ] **R3 -- SDD-020.R6 closed: triple-destination log writers.**
...
- [ ] **R4 -- SDD-020.R8 closed: dedup invoked from /triage and /clarify.**
...
- [ ] **R5 -- No regression in existing test count.**
...
- [ ] **R7 -- `exec/state.md` regenerates cleanly.**
...
- [ ] **O1 -- Dedup log JSON schema documented.**
  ...
  source.
- [ ] **O2 -- Queue ordering visible in `fleet.py lock status` output.**
...
- [ ] **Unit (R1)**: queue ordering with mixed priorities, FIFO tiebreak,
  empty-queue case. Files: `cli/test_fleet.py`.
- [ ] **Unit (R2)**: grandfather pre-cutover, post-cutover, mixed.
  Files: `cli/test_fleet.py`.
- [ ] **Unit (R3)**: each log writer in isolation + integration through
  `cmd_scan`; `--no-emit-logs` regression. Files: `cli/test_dedup.py`.
- [ ] **Markdown lint (R4)**: prompt files contain expected literal
  substring + tier-action guidance. Files: `cli/test_dedup.py` (new
  `TestPromptHooks` class) OR a dedicated `cli/test_prompts.py` (plan
  decides).
- [ ] **Regression (R5, R6)**: full suite green; `schema_lint` clean;
