...
- [ ] **V56-1:** Unit tests prove PI-1..PI-9 render once in numeric order from live
...
- [ ] **V56-2:** Unit tests prove missing-marker/no-active fallback and injector
  idempotence. Proves AC56-2.
- [ ] **V56-3:** Exact-wording test proves only Sprint 5's
...
- [ ] **V56-4:** Real generated `exec/state.html` has PI-9 as the sole current
  pill and no stale PI current. Proves AC56-1/AC57-3.

## REQUIRED -- SDD-057

- [ ] **V57-1:** Loader unit tests cover: highest-numbered ACTIVE PI wins; inactive
...
- [ ] **V57-2:** Boundary tests reject CLOSED/DONE/PROPOSED, malformed, absent,
  duplicate-conflicting, and unreadable markers without guessing. Proves AC57-1.
- [ ] **V57-3:** Build integration test spies on the unchanged
...
- [ ] **V38-4:** Accessibility test/manual evidence proves visible state labels and
...
- [ ] **VX-1:** TDD evidence records RED before production behavior and GREEN after
  for each state-builder behavior group and the wording guard. Proves RX-1.
- [ ] **VX-2:** `TestS1FootprintLockGuard` passes unchanged; independent
...
- [ ] **VX-3:** Full `python -m pytest spec-driven-development/ --tb=no -q` is
