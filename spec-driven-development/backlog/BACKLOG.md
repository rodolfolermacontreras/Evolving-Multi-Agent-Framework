...
| SDD-015 | Model upgrades as Level-2 decisions w/ regression-test branch + A/B + cost analysis | P1 | H | H | H | S | -- | PI-5 Sprint 4 | **DONE** 2026-06-08. Commit chain: `c3ac624` (design/planning) -> `0b47def` (F-14 implementation) -> `a2c1476` (ADR-016 acceptance/governance unblock) -> `dbfe3c6` (F-14 close) -> F-15 close commit. Shipped model-upgrade protocol, stdlib no-network A/B harness, pricing/workload fixtures, accepted ADR-016, `decision-policy.md` cross-reference, and validation 12/12 REQUIRED checked. |
