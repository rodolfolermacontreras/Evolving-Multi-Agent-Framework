...
| SDD-049 | True file-overlap conflict detector for fleet dispatch (replaces the manual per-worker file-scope discipline) | P3 | 3 | 2 | 0.7 | 5 | -- | Unscheduled | OPEN -- filed 2026-06-26 from SDD-047 D-3. Today `fleet.py` performs only the serial CLARIFY/SPEC gate (`_scan_lock_state`, SDD-019); avoiding same-file dispatch within a batch is a manual discipline (explicit per-worker file scopes), not an automated check. This item, if wanted, builds a real pre-dispatch file-overlap detector that reads each worker brief's IN-scope file set and blocks/warns on intersections before dispatch. Honest placeholder so the "conflict detection" over-claim removed from `GENERALIZATION_SDD.md` + `constitution/roadmap.md` is tracked, not papered over. |
