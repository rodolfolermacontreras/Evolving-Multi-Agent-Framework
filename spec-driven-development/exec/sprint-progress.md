...
- **state_builder feature-stage heuristic gap (P3)**: `exec/state.md` showed `friction-analysis-template` at `IMPLEMENT 0/12` even though SDD-014 closed DONE at `7438bfd`. The state_builder feature-stage detection appears to read tasks.md task counts; features that close without a tasks.md (or with tasks.md absent because the work was skip-to-implement per spec-sizing) report incorrectly. The FDC backfill set `status: done` in the frontmatter of `2026-06-03-friction-analysis-template/spec.md` and `validation.md`; once state_builder is taught to prefer the frontmatter `status: done` over tasks.md task counts (or to suppress IMPLEMENT N/M for features that legitimately have no tasks.md), the display gap closes. Out of scope for Sprint 4.
